...
function buildNavItems(emergencyCount: number): NavItem[] {
  return [
    {
      label: 'Overview',
      href: '/dashboard',
...
export default function Sidebar({ emergencyCount = 0 }: { emergencyCount?: number }) {
  const pathname = usePathname()
  const router = useRouter()
  const supabase = createClient()
  const navItems = buildNavItems(emergencyCount)
    ...
    <aside className="w-64 bg-serenity-900 text-white flex flex-col h-full">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {item.icon}
              {item.label}
              {/* Live emergency count badge — hidden when 0 */}
              {item.alertCount !== undefined && item.alertCount > 0 && (
                <span className="ml-auto min-w-[20px] h-5 px-1 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center animate-pulse">
      ...
      <div className="p-4 border-t border-serenity-800">
        ...
        <button
          ...
          className="mt-3 w-full text-left text-xs text-serenity-400 hover:text-white transition flex items-center gap-2"
        >
